var getUser = function(wikiObject){

	// In this view, we are searching across all wikis for a certain user. 
	// We ingest the logs of all the wikis, and then we say, before rowNames gets populated, 
	// that we want only users = User. 
	makeUserVis(wikiObject)
}


	var makeUserVis = function(vikiObject){
		getUserIWTable(vikiObject)
	}

	function buildUserHeatmap(vikiObject){
		var gs;
		vikiObject.colName === 'day' ? gs = 32 : gs = 32;
		var margin = { top: 100, right: 100, bottom: 100, left: 100 },
	          width = 2000 - margin.left - margin.right,
	          gridSize = Math.floor(width / gs),
	          expanse = gridSize * vikiObject.objAry.colNames.length -200,
	...
	//          times = ["1a", "2a", "3a", "4a", "5a", "6a", "7a", "8a", "9a", "10a", "11a", "12a", "1p", "2p", "3p", "4p", "5p", "6p", "7p", "8p", "9p", "10p", "11p", "12p"];
		...
		// filtering user array
		var userAry = vikiObject.objAry.rowNames.filter(function(elem, pos) {
			    return vikiObject.objAry.rowNames.indexOf(elem) == pos;
		    }); 

		// filtering logs
		var userData = vikiObject.objAry.filter(function(square){
			return square.row === vikiObject.user
		})
		...
		var filData = userData.filter(filterDays)
		...
		// making a list of wikis to display in the tooltip
	...
	///////////////////////////////////////////////////
		...
		// var userHtml = "<h2>User: " + vikiObject.user + " </h2>"
	    ...
	    //           .attr("text-anchor", "start")
		   ...
		   //        });
	          ...
	          var heatMap = svg.selectAll(".hour")
	              .data(data)
	              .enter().append("rect")
	              .attr("y", function(d,i) { return (x(d.row)) * gridSize; })
	...
	function propogateStatsUsers(vikiObject){
	...
	//			l.month = date.getMonth();
		...
		vikiObject.objAry = populateUsers(allLogs, vikiObject.rowName, 'daysAgo');

		// the two keys to filter on
		function populateUsers(ary, key1, key2){
			var lAry = [];
			var result = {};
			lAry.rowNames = [];
			lAry.colNames = [];
			...
			ary.forEach(function(l){
				...
				if(obj.length < 1){
					...
					lObject.logs.push(l)
		  		    lAry.push(lObject)
		 		    // lAry.rowNames.indexOf(val1) === -1 && 
		 		    lAry.rowNames.push(vikiObject.user);
	...
	function populateUserWikiLogs(vikiObject){
		vikiObject.i = 0;
		...
		incrementalRecursiveQueryUsers(vikiObject);
	};
	...
	function incrementalRecursiveQueryUsers(vikiObject){
		...
		if(vikiObject.i !== vikiObject.activeWikis.length){
			...
			propogateStatsUsers(vikiObject);
			buildUserHeatmap(vikiObject)
		}
	...
	window.getUserIWTable = function(vikiObject){
	...
	//				getWikiLogs(vikiObject);
					populateUserWikiLogs(vikiObject);
	    },
	      error: function(jqXHR, textStatus, errorThrown) {
